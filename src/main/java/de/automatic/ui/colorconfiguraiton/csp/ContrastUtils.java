...
public class ContrastUtils {
	...
	/**
	 * The minimum text size considered large for contrast checking purposes, as
	 ...
	 * checking purposes, as taken from the WCAG standards at
	 * http://www.w3.org/TR/UNDERSTANDING-WCAG20/visual-audio-contrast-contrast.html
	 */
	...
	/**
	 * Calculates the luminance value of a given {@code int} representation of
	 * {@link Color}.
	 * <p>
	 * Derived from formula at
	 * http://gmazzocato.altervista.org/colorwheel/algo.php
	 *
	 * @param color
	 *            The {@link Color} to evaluate
	...
	public static double calculateLuminance(int r, int g, int b) {
		...
		sRGB[0] = ((double) r) / 255.0d;
		sRGB[1] = ((double) g)  / 255.0d;
		sRGB[2] = ((double) b)  / 255.0d;
	 ...
	 *
	 * @param colors
	 *            Collection of {@link Color} values to convert
	...
	public static CharSequence colorsToHexString(Iterable<Integer> colors) {
		...
		for (int color : colors) {
			...
			colorStr.append(colorToHexString(color));
		}
	...
	/**
	 * Converts an {@code int} representation of a {@link Color} to a hex
	 * string.
	 *
	 * @param color
	 *            The {@link Color} value to convert
